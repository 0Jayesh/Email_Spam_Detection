...
def train_model():
    ...
    # --- Validation of Solver and Penalty ---
    ...
    # Extract solver and penalty used for tuning
    test_solver = param_grid_to_use.get('lr__solver', ['liblinear'])[0]
    test_penalty = param_grid_to_use.get('lr__penalty', ['l2'])[0]
    
    # Check if the combination is valid
    valid_penalties = supported_combinations.get(test_solver, [])
    if test_penalty not in valid_penalties:
        return jsonify({
            "error": f"Unsupported combination: solver '{test_solver}' does not support penalty '{test_penalty}' in scikit-learn."
        }), 400
    # ----------------------------------------
